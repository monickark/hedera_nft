...
async function createClient1() {
    let client;
    try {
        //Grab your Hedera testnet account ID and private key from your .env file
        console.log("Inside create client1...");
        const myAccountId = process.env.MY_ACCOUNT_ID1;
        const myPrivateKey = process.env.MY_PRIVATE_KEY1;
        console.log("myAccountId: "+ myAccountId);
        console.log("myPrivateKey: "+ myPrivateKey);
        // If we weren't able to grab it, we should throw a new error
        if (myAccountId == null ||
